							...
							<form method="POST" action="{{ route('cart.checkout.store') }}">
								...
								<div class="clearfix row">
									<div class="col-lg-6 col-md-6 col-sm-12 form-group">
										<input type="text" name="name" id="name" placeholder="Masukkan Nama" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror"  required>
                                        @error('name')
									...
									<div class="col-lg-6 col-md-6 col-sm-12 form-group">
										<input type="text" name="telephone" id="telephone" value="{{ old('telephone') }}" placeholder="Masukan Nomor Handphone" class="form-control @error('telephone') is-invalid @enderror" required>
                                        @error('telephone')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
									</div>
									...
									<div class="col-lg-4 col-md-4 col-sm-12 form-group">
										<select class="custom-select-box @error('province_id') is-invalid @enderror" name="province_id" id="province_id" required>
                                    ...
                                    <div class="col-lg-4 col-md-4 col-sm-12 form-group">
										<select name="city_id" class="custom-select-box @error('city_id') is-invalid @enderror" id="city_id" required>
											<option>Kota</option>
                                    ...
                                    <div class="col-lg-4 col-md-4 col-sm-12 form-group">
										<select name="subdistrict" class="custom-select-box @error('subdistric') is-invalid @enderror" id="subditric" required>
											<option>Kecamatan</option>
										</select>
									</div>

                                    <div class="col-lg-12 col-md-12 col-sm-12 form-group">
										<input type="text" name="address" id="address" value="{{ old('address') }}" placeholder="Alamat Lengkap" required>
									</div>

                                    <div class="col-lg-12 col-md-12 col-sm-12 form-group">
										<select name="courier_id" class="custom-select-box @error('courier_id') is-invalid @enderror" id="courier_id">
											<option value="">Pilih Kurir</option>
                                            <option value="ambil">Ambil ditoko</option>
                                            @forelse ($couriers as $courier)
                                                @if ($courier->status == 1)
                                                <option value="{{ $courier->id }}" {{ old('courier_id', $courier->id) ==
                                                    $courier->id ? 'selected':'' }}>{{ $courier->name }}</option>
                                                @endif
                                            @empty
                                            <option value="">Tidak ada data</option>
                                            @endforelse
		...
		$.ajax({
			...
			success: function(res) {
				...
				subdistrict.forEach(sub => {
					$('select[name=subdistrict]').append(`<option value="${sub.subdistrict_id}">${sub.subdistrict_name}</option>`);
				});
